...
以降の手順を実行する前に**以下のドキュメントの内容に従い、ssoTabApp という名前でアプリを作成しておいてください。** 
...
Teams アプリの基本情報が登録が完了後、SSO 機能を追加する手順に進みます。
...
2. サーバーサイドの開発に必要なモジュールをインストールします。
    ...
    　
        ...
        ```
3. Visual Studio Code 画面左のツリービュー内にあるフォルダ名 \[SSOTABAPP\] の右隣にある \[新しいファイル\]アイコンをクリックし、表示されたテキストボックスに index.html と入力してファイル **.env** を作成します
