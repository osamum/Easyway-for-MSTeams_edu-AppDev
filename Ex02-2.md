...
9. **index.css** ファイルが作成され編集状態になるのでエディタ画面に以下のコードを貼り付けます
    ...
    li {cursor: pointer;}
    .selectedIem{background-color: #DCDFFA;}
...
9. 記述したファイル名 aadAuth.js を\[Crtl\] キーを押下しながらクリックすると"**'aadAuth.js を開くことができません'**" と書かれたメッセージボックスが表示されるので \[**ファイルの作成**\] ボタンをクリックします
    ...
    <div id="ground">
        ...
        <div style="display:flex;">
            ...
            <div id="loginedArea" style="display: none;">
                <a href="javascript:showProfileDialog()">プロファイル情報の表示</a>
                <br>
...
18. index.html に追加した タグ(UI)を操作するための簡単なコードを追加します。

    body の**閉じタグ**下の行で、 **script** とタイプして表示される候補から **script:src** を選択します

19. script タグが挿入され src 属性の " (ダブルクォーテーション) 内にカーソルが移動するので **index.js** と記述します

20. 記述したファイル名 index.js を\[Crtl\] キーを押下しながらクリックすると"**'index.js を開くことができません'**" と書かれたメッセージボックスが表示されるので \[**ファイルの作成**\] ボタンをクリックします
